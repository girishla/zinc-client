...
const minutesOptions = toOptions(range(1, 60)).map((option: IOption) => {
...
const hoursOptions = toOptions(range(0, 24)).map((option: IOption) => {
  const { label, value } = option;
  return {
...
export default class PeriodicallyFrameTab extends PresetTab {
  ...
  public isMinutesMultiple = () => {
    const { minutesMultiple } = this.state;
    return minutesMultiple;
  };

  public onHoursChange = (field: string) => {
    return (value: string) => {
