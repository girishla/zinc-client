...
describe("CronBuilder", () => {
  it("initial render", () => {
    const wrapper: any = mount(<CronBuilder />);
    expect(wrapper.find("legend").find(Tab)).toHaveLength(3);
    expect(wrapper.find(PeriodicallyTab)).toHaveLength(1);
  });

  it("should parse expression", () => {
    const wrapper = mount(<CronBuilder cronExpression="5,15,25 */2 * * 1-5" />);
    expect((wrapper.instance() as any).presetComponent.state).toEqual({
      minutes: ["5", "15", "25"],
      hours: "2",
      dayOfWeek: ["1-5"],
      dayOfMonth: EVERY,
  ...
  it("should call onChange", () => {
    ...
    const wrapper = mount(
      <CronBuilder cronExpression={expression} onChange={onChange} />
    );
  ...
  it("should set active tab 1", () => {
    const wrapper = mount(<CronBuilder cronExpression={"25 17-21 4 2 6-7"} />);
    expect((wrapper.instance() as any).presetComponent.state).toEqual({
      minutes: ["25"],
      hours: "17-21",
      hoursFrom: "17",
  ...
  it("should correctly set state for the 3rd tab", () => {
    const wrapper = mount(<CronBuilder cronExpression={"48 6 24 6 2"} />);
    wrapper
      .find("legend")
      .find(Tab)
      .at(2)
  ...
  it("should not show result if flag is false", () => {
    const wrapper = mount(
      <CronBuilder cronExpression={"48 6 24 6 2"} showResult={false} />
    );
    wrapper.find("[data-action]").simulate("click");
  ...
  it("periodically tab should parse range hours to single", () => {
    const wrapper = mount(
      <CronBuilder cronExpression={"48 12-20 24 6 2"} showResult={false} />
    );
    wrapper
      .find(Tab)
      .at(0)
